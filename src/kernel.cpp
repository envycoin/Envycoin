...
static bool GetLastStakeModifier(const CBlockIndex* pindex, uint64_t& nStakeModifier, int64_t& ENVodifierTime)
{
    if (!pindex)
    ...
    ENVodifierTime = pindex->GetBlockTime();
    return true;
}
...
static int64_t GetStakeModifierSelectionIntervalSection(int nSection)
{
    ...
    return (ENVodifierInterval * 63 / (63 + ((63 - nSection) * (MODIFIER_INTERVAL_RATIO - 1))));
}
...
bool ComputeNextStakeModifier(const CBlockIndex* pindexPrev, uint64_t& nStakeModifier, bool& fGeneratedStakeModifier)
{
    ...
    int64_t ENVodifierTime = 0;
    if (!GetLastStakeModifier(pindexPrev, nStakeModifier, ENVodifierTime))
        return error("ComputeNextStakeModifier: unable to get last modifier");
    if (fDebug)
    {
        printf("ComputeNextStakeModifier: prev modifier=0x%016"PRIx64" time=%s\n", nStakeModifier, DateTimeStrFormat(ENVodifierTime).c_str());
    }
    if (ENVodifierTime / ENVodifierInterval >= pindexPrev->GetBlockTime() / ENVodifierInterval)
        return true;

    // Sort candidate blocks by timestamp
    vector<pair<int64_t, uint256> > vSortedByTimestamp;
    vSortedByTimestamp.reserve(64 * ENVodifierInterval / nTargetSpacing);
    int64_t nSelectionInterval = GetStakeModifierSelectionInterval();
    int64_t nSelectionIntervalStart = (pindexPrev->GetBlockTime() / ENVodifierInterval) * ENVodifierInterval - nSelectionInterval;
    ...
    if (fDebug && GetBoolArg("-printstakemodifier"))
    {
        string strSelectioENVap = "";
        // '-' indicates proof-of-work blocks not selected
        strSelectioENVap.insert(0, pindexPrev->nHeight - nHeightFirstCandidate + 1, '-');
        pindex = pindexPrev;
        while (pindex && pindex->nHeight >= nHeightFirstCandidate)
        {
            // '=' indicates proof-of-stake blocks not selected
            if (pindex->IsProofOfStake())
                strSelectioENVap.replace(pindex->nHeight - nHeightFirstCandidate, 1, "=");
            pindex = pindex->pprev;
        }
        BOOST_FOREACH(const PAIRTYPE(uint256, const CBlockIndex*)& item, mapSelectedBlocks)
        {
            ...
            strSelectioENVap.replace(item.second->nHeight - nHeightFirstCandidate, 1, item.second->IsProofOfStake()? "S" : "W");
        }
        printf("ComputeNextStakeModifier: selection height [%d, %d] map %s\n", nHeightFirstCandidate, pindexPrev->nHeight, strSelectioENVap.c_str());
    }
    if (fDebug)
    {
