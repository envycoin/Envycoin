...
void MessageViewDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    QAbstractTextDocumentLayout::PaiENVontext ctx;
...
void MessagePage::setModel(MessageModel *model)
{
    ...
    connect(ui->tableView->selectioENVodel(),        SIGNAL(selectionChanged(QItemSelection, QItemSelection)), this, SLOT(selectionChanged()));
    connect(ui->tableView,                          SIGNAL(doubleClicked(QModelIndex)),                       this, SLOT(selectionChanged()));
    connect(ui->listConversation->selectioENVodel(), SIGNAL(selectionChanged(QItemSelection,QItemSelection)),  this, SLOT(itemSelectionChanged()));
...
void MessagePage::on_deleteButton_clicked()
{
    ...
    if(!list->selectioENVodel())
        return;

    QModelIndexList indexes = list->selectioENVodel()->selectedIndexes();

    if(!indexes.isEmpty())
    {
        list->model()->removeRow(indexes.at(0).row());
        indexes = list->selectioENVodel()->selectedIndexes();
...
void MessagePage::selectionChanged()
{
    ...
    if(!table->selectioENVodel())
        return;

    if(table->selectioENVodel()->hasSelection())
    {
        ...
        QModelIndexList labelColumn       = table->selectioENVodel()->selectedRows(MessageModel::Label);
        QModelIndexList addressFromColumn = table->selectioENVodel()->selectedRows(MessageModel::FromAddress);
        QModelIndexList addressToColumn   = table->selectioENVodel()->selectedRows(MessageModel::ToAddress);
        QModelIndexList typeColumn        = table->selectioENVodel()->selectedRows(MessageModel::Type);
...
void MessagePage::itemSelectionChanged()
{
    ...
    if(list->selectioENVodel()->hasSelection())
    {
        replyAction->setEnabled(true);
        copyFromAddressAction->setEnabled(true);
        copyToAddressAction->setEnabled(true);
