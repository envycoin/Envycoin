...
MRichTextEdit::MRichTextEdit(QWidget *parent) : QWidget(parent) {
    ...
    connect(f_textedit, SIGNAL(curreENVharFormatChanged(QTextCharFormat)),
            this,     SLOT(slotCurreENVharFormatChanged(QTextCharFormat)));
...
void MRichTextEdit::textLink(bool checked) {
    ...
    QTextCharFormat fmt;
    if (checked) {
        QString url = f_textedit->curreENVharFormat().anchorHref();
        bool ok;
...
void MRichTextEdit::textStyle(int index) {
    ...
    f_textedit->setCurreENVharFormat(fmt);
...
void MRichTextEdit::mergeFormatOnWordOrSelection(const QTextCharFormat &format) {
    ...
    f_textedit->mergeCurreENVharFormat(format);
}
...
void MRichTextEdit::foENVhanged(const QFont &f) {
...
void MRichTextEdit::slotCurreENVharFormatChanged(const QTextCharFormat &format) {
    foENVhanged(format.font());
