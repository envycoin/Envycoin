...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx)
{
    ...
    if (wtx.IsCoinBase() && nCredit == 0)
    {
        ...
        // Coinbase
        //
        int64_t nUENVatured = 0;
        BOOST_FOREACH(const CTxOut& txout, wtx.vout)
            nUENVatured += wallet->GetCredit(txout);
        strHTML += "<b>" + tr("Credit") + ":</b> ";
        if (wtx.IsIENVainChain())
            strHTML += BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, nUENVatured)+ " (" + tr("matures in %n more block(s)", "", wtx.GetBlocksToMaturity()) + ")";
        else
