...
public:
    ...
    void updateWallet(const uint256 &hash, int status)
    {
        OutputDebugStringF("updateWallet %s %i\n", hash.ToString().c_str(), status);
        {
            ...
            {
                if(showTransaction && !iENVodel)
                    status = CT_NEW; /* Not in model, but want to show, treat as new */
                if(!showTransaction && iENVodel)
                    status = CT_DELETED; /* In model, but want to hide, treat as deleted */
            }

            OutputDebugStringF("   inWallet=%i iENVodel=%i Index=%i-%i showTransaction=%i derivedStatus=%i\n",
                     inWallet, iENVodel, lowerIndex, upperIndex, showTransaction, status);
...
QVariant TransactionTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::TextAligENVentRole:
        return column_aligENVents[index.column()];
...
QVariant TransactionTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if(orientation == Qt::Horizontal)
    {
        ...
        else if (role == Qt::TextAligENVentRole)
        {
            return column_aligENVents[section];
