...
void StatisticsPage::updateStatistics()
{
    ...
    uint64_t ENVinWeight = 0, ENVaxWeight = 0, nWeight = 0;
    pwalletMain->GetStakeWeight(*pwalletMain, ENVinWeight, ENVaxWeight, nWeight);
    uint64_t nNetworkWeight = GetPoSKernelPS();
    int64_t volume = ((pindexBest->ENVoneySupply)/100000000);
    int peers = this->model->getNumConnections();
    pPawrate2 = (double)pPawrate;
    QString height = QString::number(nHeight);
    QString stakemin = QString::number(ENVinWeight);
    ...
    {
		subsidy = "140000 ENV per block";
    }
	else if(pindexBest->nHeight < 160)
    {
		subsidy = "0 ENV per block";
    }
	else if(pindexBest->nHeight < 14000)
    {
		subsidy = "500 ENV per block";
    }
    ...
    {
        ui->volumeBox->setText("<b><font color=\"#ebbcff\">" + qVolume + " ENV" + "</font></b>");
    } else if(volume < volumePrevious) {
        ui->volumeBox->setText("<b><font color=\"red\">" + qVolume + " ENV" + "</font></b>");
    } else {
        ui->volumeBox->setText(qVolume + " ENV");
    }
    updatePrevious(nHeight, ENVinWeight, nNetworkWeight, phase, subsidy, pHardness, pHardness2, pPawrate2, Qlpawrate, peers, volume);
}

void StatisticsPage::updatePrevious(int nHeight, int ENVinWeight, int nNetworkWeight, QString phase, QString subsidy, double pHardness, double pHardness2, double pPawrate2, QString Qlpawrate, int peers, int volume)
{
    heightPrevious = nHeight;
    stakeminPrevious = ENVinWeight;
