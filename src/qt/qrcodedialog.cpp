...
{
    QString ret = QString("Envycoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (!ui->lENVessage->text().isEmpty())
    {
        QString msg(QUrl::toPercentEncoding(ui->lENVessage->text()));
...
void QRCodeDialog::on_lENVessage_textChanged()
{
    genCode();
}
