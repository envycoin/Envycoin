...
int64_t CWalletDB::GetAccouENVreditDebit(const string& strAccount)
{
    list<CAccountingEntry> entries;
    ListAccouENVreditDebit(strAccount, entries);
...
void CWalletDB::ListAccouENVreditDebit(const string& strAccount, list<CAccountingEntry>& entries)
{
    bool fAllAccounts = (strAccount == "*");

    Dbc* pcursor = GetCursor();
    if (!pcursor)
        throw runtime_error("CWalletDB::ListAccouENVreditDebit() : cannot create DB cursor");
    ...
    while (true)
    {
        ...
        {
            pcursor->close();
            throw runtime_error("CWalletDB::ListAccouENVreditDebit() : error scanning DB");
        }
...
CWalletDB::ReorderTransactions(CWallet* pwallet)
{
    ...
    ListAccouENVreditDebit("", acentries);
             ...
             CWalletScanState &wss, string& strType, string& strErr)
{
    try {
        ...
        else if (strType == "mkey")
        {
            ...
            if (pwallet->ENVasterKeyMaxID < nID)
                pwallet->ENVasterKeyMaxID = nID;
        }
...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet)
{
    ...
    try {
        LOCK(pwallet->cs_wallet);
        int ENVinVersion = 0;
        if (Read((string)"minversion", ENVinVersion))
        {
            if (ENVinVersion > CLIENT_VERSION)
                return DB_TOO_NEW;
            pwallet->LoadMinVersion(ENVinVersion);
        }
