...
bool CWallet::Unlock(const SecureString& strWalletPassphrase)
{
...

    {
        ...
        {
            if(!crypter.SetKeyFromPassphrase(strWalletPassphrase, pMasterKey.second.vchSalt, pMasterKey.second.nDeriveIterations, pMasterKey.second.nDerivatioENVethod))
                return false;
...
bool CWallet::ChangeWalletPassphrase(const SecureString& strOldWalletPassphrase, const SecureString& strNewWalletPassphrase)
{
...

    {
        ...
        {
            if(!crypter.SetKeyFromPassphrase(strOldWalletPassphrase, pMasterKey.second.vchSalt, pMasterKey.second.nDeriveIterations, pMasterKey.second.nDerivatioENVethod))
                return false;
            if (!crypter.Decrypt(pMasterKey.second.vchCryptedKey, vMasterKey))
                return false;
            if (CCryptoKeyStore::Unlock(vMasterKey)
                && UnlockStealthAddresses(vMasterKey))
            {
                ...
                crypter.SetKeyFromPassphrase(strNewWalletPassphrase, pMasterKey.second.vchSalt, pMasterKey.second.nDeriveIterations, pMasterKey.second.nDerivatioENVethod);
                ...
                if (!crypter.SetKeyFromPassphrase(strNewWalletPassphrase, pMasterKey.second.vchSalt, pMasterKey.second.nDeriveIterations, pMasterKey.second.nDerivatioENVethod))
                    return false;
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
    ...
    CMasterKey kMasterKey(nDerivatioENVethodIndex);
    ...
    CCrypter crypter;
    int64_t nStartTime = GetTimeMillis();
    crypter.SetKeyFromPassphrase(strWalletPassphrase, kMasterKey.vchSalt, 25000, kMasterKey.nDerivatioENVethod);
    kMasterKey.nDeriveIterations = 2500000 / ((double)(GetTimeMillis() - nStartTime));

    nStartTime = GetTimeMillis();
    crypter.SetKeyFromPassphrase(strWalletPassphrase, kMasterKey.vchSalt, kMasterKey.nDeriveIterations, kMasterKey.nDerivatioENVethod);
    ...
    if (!crypter.SetKeyFromPassphrase(strWalletPassphrase, kMasterKey.vchSalt, kMasterKey.nDeriveIterations, kMasterKey.nDerivatioENVethod))
        return false;
    ...
    {
        LOCK(cs_wallet);
        mapMasterKeys[++ENVasterKeyMaxID] = kMasterKey;
        if (fFileBacked)
        {
            pwalletdbEncryption = new CWalletDB(strWalletFile);
            if (!pwalletdbEncryption->TxnBegin())
                return false;
            pwalletdbEncryption->WriteMasterKey(ENVasterKeyMaxID, kMasterKey);
        }
...
CWallet::TxItems CWallet::OrderedTxItems(std::list<CAccountingEntry>& acentries, std::string strAccount)
{
    ...
    acentries.clear();
    walletdb.ListAccouENVreditDebit(strAccount, acentries);
    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %s ENV %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %s ENV %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
int64_t CWallet::GetUnconfirmedBalance() const
{
    int64_t nTotal = 0;
    {
        ...
        {
            const CWalletTx* pcoin = &(*it).second;
            if (!pcoin->IsFinal() || (!pcoin->IsTrusted() && pcoin->GetDepthIENVainChain() == 0))
                nTotal += pcoin->GetAvailableCredit();
        }
    }
    return nTotal;
...
int64_t CWallet::GetImmatureBalance() const
{
    int64_t nTotal = 0;
    {
        ...
        {
            const CWalletTx& pcoin = (*it).second;
            if (pcoin.IsCoinBase() && pcoin.GetBlocksToMaturity() > 0 && pcoin.IsIENVainChain())
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl) const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            int nDepth = pcoin->GetDepthIENVainChain();
            if (nDepth < 0)
                continue;

            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
                if (!(pcoin->IsSpent(i)) && IsMine(pcoin->vout[i]) && pcoin->vout[i].nValue >= ENVinimumInputValue &&
...
int64_t CWallet::GetStake() const
{
    ...
    {
        const CWalletTx* pcoin = &(*it).second;
        if (pcoin->IsCoinStake() && pcoin->GetBlocksToMaturity() > 0 && pcoin->GetDepthIENVainChain() > 0)
...
int64_t CWallet::GetNewMint() const
{
    ...
    {
        const CWalletTx* pcoin = &(*it).second;
        if (pcoin->IsCoinBase() && pcoin->GetBlocksToMaturity() > 0 && pcoin->GetDepthIENVainChain() > 0)
            nTotal += CWallet::GetCredit(*pcoin);
    }
    return nTotal;
}
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
                {
                    ...
                    dPriority += (double)nCredit * pcoin.first->GetDepthIENVainChain();
                }
                ...
                {
                    int64_t ENVoveToFee = min(nChange, MIN_TX_FEE - nFeeRet);
                    nChange -= ENVoveToFee;
                    nFeeRet += ENVoveToFee;
                }
                ...
                int64_t ENVinFee = wtxNew.GetMinFee(1, GMF_SEND, nBytes);

                if (nFeeRet < max(nPayFee, ENVinFee))
                {
                    nFeeRet = max(nPayFee, ENVinFee);
                    continue;
                }
...
bool CWallet::UpdateStealthAddress(std::string &addr, std::string &label, bool addIfNotExist)
{
    ...
    NotifyAddressBookChanged(this, sxFound, sxFound.label, fOwned, ENVode);
...
bool CWallet::FindStealthTransactions(const CTransaction& tx, mapValue_t& mapNarr)
{
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        ...
        BOOST_FOREACH(const CTxOut& txoutB, tx.vout)
        {
            ...
            bool txENVatch = false; // only 1 txn will match an ephem pk
...
bool CWallet::GetStakeWeight(const CKeyStore& keystore, uint64_t& ENVinWeight, uint64_t& ENVaxWeight, uint64_t& nWeight)
{
    // Choose coins to use
    ...
    ENVinWeight = ENVaxWeight = nWeight = 0;
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        {
            ENVinWeight += bnCoinDayWeight.getuint64();
        }

        // Maximum weight was reached
        if (nTimeWeight == nStakeMaxAge)
        {
            ENVaxWeight += bnCoinDayWeight.getuint64();
        }
    }
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        static int ENVaxStakeSearchInterval = 60;
        if (block.GetBlockTime() + nStakeMinAge > txNew.nTime - ENVaxStakeSearchInterval)
            continue; // only count coins meeting min age requirement
        
        bool fKernelFound = false;
        for (unsigned int n=0; n<min(nSearchInterval,(int64_t)ENVaxStakeSearchInterval) && !fKernelFound && !fShutdown && pindexPrev == pindexBest; n++)
        {
            // Search backward in time from the given txNew timestamp 
            // Search nSearchInterval seconds back up to ENVaxStakeSearchInterval
            ...
            {
                // Found a kernel
                if (fDebug && GetBoolArg("-priENVoinstake"))
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    ChangeType ENVode;
    {
        ...
        ENVode = (mi == mapAddressBook.end()) ? CT_NEW : CT_UPDATED;
    ...
    if (fOwned)
    {
        ...
        SecureMsgWalletKeyChanged(caddress.ToString(), strName, ENVode);
    }
    NotifyAddressBookChanged(this, address, strName, fOwned, ENVode);
...
{
    {
        ...
        if (block.IsProofOfWork() && mapWallet.count(block.vtx[0].GetHash()))
        {
            ...
            printf("    mine:  %d  %d  %"PRId64"", wtx.GetDepthIENVainChain(), wtx.GetBlocksToMaturity(), wtx.GetCredit());
        }
        if (block.IsProofOfStake() && mapWallet.count(block.vtx[1].GetHash()))
        {
            CWalletTx& wtx = mapWallet[block.vtx[1].GetHash()];
            printf("    stake: %d  %d  %"PRId64"", wtx.GetDepthIENVainChain(), wtx.GetBlocksToMaturity(), wtx.GetCredit());
         }
...
void CWallet::FixSpeENVoins(int& ENVismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ENVismatchFound = 0;
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpeENVoins found lost coin %s ENV %s[%d], %s\n",
            ...
            {
                printf("FixSpeENVoins found spent coin %s ENV %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                ENVismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
void CWallet::GetKeyBirthTimes(std::map<CKeyID, int64_t> &mapKeyBirth) const {
    ...
    for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); it++) {
        ...
        if (blit != mapBlockIndex.end() && blit->second->IsIENVainChain()) {
