...
public:
    ...
    bool GetStakeWeight(const CKeyStore& keystore, uint64_t& ENVinWeight, uint64_t& ENVaxWeight, uint64_t& nWeight);
    ...
    bool IsMine(const CTransaction& tx) const
    {
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
            if (IsMine(txout) && txout.nValue >= ENVinimumInputValue)
                return true;
        return false;
    }
    ...
    void FixSpeENVoins(int& ENVismatchSpent, int64_t& nBalanceInQuestion, bool fCheckOnly = false);
...
public:
    ...
    bool IsTrusted() const
    {
        ...
        if (!IsFinal())
            return false;
        int nDepth = GetDepthIENVainChain();
        if (nDepth >= 1)
            return true;
        if (nDepth < 0)
            return false;
        ...
        for (unsigned int i = 0; i < vWorkQueue.size(); i++)
        {
            ...
            if (!ptx->IsFinal())
                return false;
            int nPDepth = ptx->GetDepthIENVainChain();
            if (nPDepth >= 1)
                continue;
            if (nPDepth < 0)
                return false;
