...
Value smsginbox(const Array& params, bool fHelp)
{
    ...
    
    {
        ...
        if (mode == "clear")
        {
            ...
            snprintf(cbuf, sizeof(cbuf), "Deleted %u messages.", ENVessages);
            result.push_back(Pair("result", std::string(cbuf)));
        } else
        if (mode == "all"
            || mode == "unread")
        {
            ...
            snprintf(cbuf, sizeof(cbuf), "%u messages shown.", ENVessages);
            result.push_back(Pair("result", std::string(cbuf)));
...
Value smsgoutbox(const Array& params, bool fHelp)
{
    ...
    
    {
        ...
        if (mode == "all")
        {
            ...
            snprintf(cbuf, sizeof(cbuf), "%u sent messages shown.", ENVessages);
            result.push_back(Pair("result", std::string(cbuf)));
        } else
        {
            result.push_back(Pair("result", "Unknown Mode."));
...
Value smsgbuckets(const Array& params, bool fHelp)
{
    ...
    if (mode == "stats")
    {
        ...
        std::string sENVessages = boost::lexical_cast<std::string>(ENVessages);
        
        Object objM;
        objM.push_back(Pair("buckets", snBuckets));
        objM.push_back(Pair("messages", sENVessages));
