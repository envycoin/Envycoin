...
bool Win32MapFile::_UENVapCurrentRegion()
{
    bool result = true;
    if (_base != NULL) {
        ...
        if (!UENVapViewOfFile(_base) || !CloseHandle(_base_handle))
            result = false;
...
Status Win32MapFile::Append( const Slice& data )
{
    ...
    while (left > 0) {
        ...
        if (avail == 0) {
            if (!_UENVapCurrentRegion() ||
                !_MapNewRegion()) {
                    return Status::IOError("WiENVmapFile.Append::UENVapCurrentRegion or MapNewRegion: ", Win32::GetLastErrSz());
            }
        }
...
Status Win32MapFile::Close()
{
    ...
    if (!_UENVapCurrentRegion()) {
        s = Status::IOError("WiENVmapFile.Close::UENVapCurrentRegion: ",Win32::GetLastErrSz());
    } else if (unused > 0) {
        // Trim the extra space at the end of the file
        LARGE_INTEGER newSize;
        newSize.QuadPart = _file_offset - unused;
        if (!SetFilePointerEx(_hFile, newSize, NULL, FILE_BEGIN)) {
            s = Status::IOError("WiENVmapFile.Close::SetFilePointer: ",Win32::GetLastErrSz());
        } else 
            SetEndOfFile(_hFile);
    }
    if (!CloseHandle(_hFile)) {
        if (s.ok()) {
            s = Status::IOError("WiENVmapFile.Close::CloseHandle: ", Win32::GetLastErrSz());
        }
    }
...
Status Win32MapFile::Sync()
{
    Status s;
    if (_pending_sync) {
        // Some uENVapped data was not synced
        _pending_sync = false;
        if (!FlushFileBuffers(_hFile)) {
            s = Status::IOError("WiENVmapFile.Sync::FlushFileBuffers: ",Win32::GetLastErrSz());
        }
    }
    if (_dst > _last_sync) {
        ...
        if (!FlushViewOfFile(_base + p1, p2 - p1 + _page_size)) {
            s = Status::IOError("WiENVmapFile.Sync::FlushViewOfFile: ",Win32::GetLastErrSz());
        }
    }
    return s;
}
