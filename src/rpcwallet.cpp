...
{
    int confirms = wtx.GetDepthIENVainChain();
...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("moneysupply",   ValueFromAmount(pindexBest->ENVoneySupply)));
    ...
    obj.push_back(Pair("mininput",      ValueFromAmount(ENVinimumInputValue)));
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress <Envycoinaddress> <amount> [comment] [comment-to]\n"
...
Value sigENVessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "sigENVessage <Envycoinaddress> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <Envycoinaddress> [minconf=1]\n"
            "Returns the total amount received by <Envycoinaddress> in transactions with at least [minconf] confirmations.");
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, wtx.vout)
            if (txout.scriptPubKey == scriptPubKey)
                if (wtx.GetDepthIENVainChain() >= ENVinDepth)
...
int64_t GetAccountBalance(CWalletDB& walletdb, const string& strAccount, int ENVinDepth)
{
    int64_t nBalance = 0;
    ...
    {
        const CWalletTx& wtx = (*it).second;
        if (!wtx.IsFinal() || wtx.GetDepthIENVainChain() < 0)
            continue;

        int64_t nReceived, nSent, nFee;
        wtx.GetAccountAmounts(strAccount, nReceived, nSent, nFee);

        if (nReceived != 0 && wtx.GetDepthIENVainChain() >= ENVinDepth && wtx.GetBlocksToMaturity() == 0)
            nBalance += nReceived;
        nBalance -= nSent + nFee;
    }

    // Tally internal accounting entries
    nBalance += walletdb.GetAccouENVreditDebit(strAccount);

    return nBalance;
}

int64_t GetAccountBalance(const string& strAccount, int ENVinDepth)
{
    CWalletDB walletdb(pwalletMain->strWalletFile);
    return GetAccountBalance(walletdb, strAccount, ENVinDepth);
}
...
Value getbalance(const Array& params, bool fHelp)
{
    ...
    if (params[0].get_str() == "*") {
        ...
        for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
        {
            ...
            if (wtx.GetDepthIENVainChain() >= ENVinDepth && wtx.GetBlocksToMaturity() == 0)
            {
    ...
    int64_t nBalance = GetAccountBalance(strAccount, ENVinDepth);
...
Value movecmd(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 3)
        // unused parameter, used to be ENVinDepth, keep type-checking it though
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom <fromaccount> <toEnvycoinaddress> <amount> [minconf=1] [narration] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        ...
        int nDepth = wtx.GetDepthIENVainChain();
        if (nDepth < ENVinDepth)
            continue;
...
void ListTransactions(const CWalletTx& wtx, const string& strAccount, int ENVinDepth, bool fLong, Array& ret)
{
    int64_t nFee;
    string strSentAccount;
    list<pair<CTxDestination, int64_t> > listReceived;
    ...
    // Received
    if (listReceived.size() > 0 && wtx.GetDepthIENVainChain() >= ENVinDepth)
    {
        bool stop = false;
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& r, listReceived)
        {
...
Value listaccounts(const Array& params, bool fHelp)
{
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        ...
        if (nDepth >= ENVinDepth && wtx.GetBlocksToMaturity() == 0)
        {
            BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& r, listReceived)
                if (pwalletMain->mapAddressBook.count(r.first))
    ...
    CWalletDB(pwalletMain->strWalletFile).ListAccouENVreditDebit("*", acentries);
...
Value listsinceblock(const Array& params, bool fHelp)
{
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); it++)
    {
        ...
        if (depth == -1 || tx.GetDepthIENVainChain() < depth)
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("Envycoin-lock-wa");

    int64_t ENVyWakeTime = GetTimeMillis() + *((int64_t*)parg) * 1000;

    ENTER_CRITICAL_SECTION(cs_nWalletUnlockTime);

    if (nWalletUnlockTime == 0)
    {
        nWalletUnlockTime = ENVyWakeTime;
    ...
    {
        if (nWalletUnlockTime < ENVyWakeTime)
            nWalletUnlockTime = ENVyWakeTime;
    }
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Return information about <Envycoinaddress>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <Envycoinpubkey>\n"
            "Return information about <Envycoinpubkey>.");
...
Value checkwallet(const Array& params, bool fHelp)
{
    ...
    pwalletMain->FixSpeENVoins(ENVismatchSpent, nBalanceInQuestion, true);
    Object result;
    if (ENVismatchSpent == 0)
        result.push_back(Pair("wallet check passed", true));
    else
    {
        result.push_back(Pair("mismatched spent coins", ENVismatchSpent));
...
Value repairwallet(const Array& params, bool fHelp)
{
    ...
    int ENVismatchSpent;
    int64_t nBalanceInQuestion;
    pwalletMain->FixSpeENVoins(ENVismatchSpent, nBalanceInQuestion);
    Object result;
    if (ENVismatchSpent == 0)
        result.push_back(Pair("wallet check passed", true));
    else
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new Envycoin stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid Envycoin stealth address."));
        return result;
    };
