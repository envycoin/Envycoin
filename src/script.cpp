...
static const size_t ENVaxNumSize = 4;


CBigNum CastToBigNum(const valtype& vch)
{
    if (vch.size() > ENVaxNumSize)
...
public:
    ...
    void Set(uint256 hash, const std::vector<unsigned char>& vchSig, const std::vector<unsigned char>& pubKey)
    {
        ...
        int64_t ENVaxCacheSize = GetArg("-maxsigcachesize", 50000);
        if (ENVaxCacheSize <= 0) return;

        LOCK(cs_sigcache);

        while (static_cast<int64_t>(setValid.size()) > ENVaxCacheSize)
        {
