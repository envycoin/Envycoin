...
extern unsigned int LimitOrphanTxSize(unsigned int ENVaxOrphans);
...
BOOST_AUTO_TEST_CASE(DoS_checkSig)
{
    ...
    msdiff = mst2 - mst1;
    long ENVanyValidate = msdiff.total_milliseconds();
    if (fDebug) printf("DoS_Checksig five: %ld\n", ENVanyValidate);

    BOOST_CHECK_MESSAGE(ENVanyValidate < nOneValidate, "Signature cache timing failed");
