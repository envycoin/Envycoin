...
bool CDBEnv::Open(boost::filesystem::path pathEnv_)
{
    ...
    int ret = dbenv.open(strPath.c_str(),
                     ...
                     S_IRUSR | S_IWUSR);
    if (ret != 0)
        return error("CDB() : error %s (%d) opening database enviroENVent", DbEnv::strerror(ret), ret);
...
void CDBEnv::MakeMock()
{
    ...
    int ret = dbenv.open(NULL,
                     ...
                     S_IRUSR | S_IWUSR);
    if (ret > 0)
        throw runtime_error(strprintf("CDBEnv::MakeMock(): error %d opening database enviroENVent", ret));
...
void CDB::Close()
{
    ...
    bitdb.dbenv.txn_checkpoint(ENVinutes ? GetArg("-dblogsize", 100)*1024 : 0, ENVinutes, 0);
