...
unsigned int ENVodifierInterval  = 10 * 60;          // time to elapse before new modifier is computed
...
unsigned int LimitOrphanTxSize(unsigned int ENVaxOrphans)
{
    unsigned int nEvicted = 0;
    while (mapOrphanTransactions.size() > ENVaxOrphans)
    {
...
int CMerkleTx::SetMerkleBranch(const CBlock* pblock)
{
    ...
    if (!pindex || !pindex->IsIENVainChain())
        return 0;
...
int64_t CTransaction::GetMinFee(unsigned int nBlockSize, enum GetMinFee_mode mode, unsigned int nBytes) const
{
    ...
    int64_t ENVinFee = (1 + (int64_t)nBytes / 1000) * nBaseFee;

    // To limit dust spam, require MIN_TX_FEE/MIN_RELAY_TX_FEE if any output is less than 0.01
    if (ENVinFee < nBaseFee)
    {
        BOOST_FOREACH(const CTxOut& txout, vout)
    ...
    if (nBlockSize != 1 && nNewBlockSize >= MAX_BLOCK_SIZE_GEN/2)
    {
        if (nNewBlockSize >= MAX_BLOCK_SIZE_GEN)
            return MAX_MONEY;
        ENVinFee *= MAX_BLOCK_SIZE_GEN / (MAX_BLOCK_SIZE_GEN - nNewBlockSize);
    }

    if (!MoneyRange(ENVinFee))
        ENVinFee = MAX_MONEY;
    return ENVinFee;
}
...
int CMerkleTx::GetDepthIENVainChainINTERNAL(CBlockIndex* &pindexRet) const
{
...
int CMerkleTx::GetDepthIENVainChain(CBlockIndex* &pindexRet) const
{
    AssertLockHeld(cs_main);
    int nResult = GetDepthIENVainChainINTERNAL(pindexRet);
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    return max(0, (nCoinbaseMaturity+10) - GetDepthIENVainChain());
}
...
int CTxIndex::GetDepthIENVainChain() const
{
    // Read block header
    CBlock block;
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (fDebug && GetBoolArg("-priENVreation"))
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    pindex->ENVint = nValueOut - nValueIn + nFees;
    pindex->ENVoneySupply = (pindex->pprev? pindex->pprev->ENVoneySupply : 0) + nValueOut - nValueIn;
...
bool CTransaction::GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        if (fDebug && GetBoolArg("-priENVoinage"))
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    // Check for ENVinDiskSpace bytes (currently 50MB)
    if (nFreeBytesAvailable < ENVinDiskSpace + nAdditionalBytes)
    {
        fShutdown = true;
        string strMessage = _("Warning: Disk space is low!");
        strMiscWarning = strMessage;
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Envycoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Envycoin FRESH POW - VARIABLE POS INTEREST - STEALTH ADDRESSES - ENV 20141508";
        CTransaction txNew;
...
void PrintBlockTree()
{
    ...
    while (!vStack.empty())
    {
        ...
        printf("%d (%u,%u) %s  %08x  %s  mint %7s  tx %"PRIszu"",
            ...
            FormatMoney(pindex->ENVint).c_str(),
            block.vtx.size());
...
bool static AlreadyHave(CTxDB& txdb, const CInv& inv)
{
    ...
    case MSG_TX:
        {
        bool txIENVap = false;
        txIENVap = mempool.exists(inv.hash);
        return txIENVap ||
...
// a large 4-byte int at any aligENVent.
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        //            msg.hdr.ENVessageSize, msg.vRecv.size(),
        //            msg.complete() ? "Y" : "N");
        ...
        // Message size
        unsigned int ENVessageSize = hdr.ENVessageSize;

        // Checksum
        CDataStream& vRecv = msg.vRecv;
        uint256 hash = Hash(vRecv.begin(), vRecv.begin() + ENVessageSize);
        unsigned int nChecksum = 0;
        memcpy(&nChecksum, &hash, sizeof(nChecksum));
        if (nChecksum != hdr.nChecksum)
        {
            printf("ProcessMessages(%s, %u bytes) : CHECKSUM ERROR nChecksum=%08x hdr.nChecksum=%08x\n",
               strCommand.c_str(), ENVessageSize, nChecksum, hdr.nChecksum);
            continue;
        }
        ...
        catch (std::ios_base::failure& e)
        {
            if (strstr(e.what(), "end of data"))
            {
                ...
                printf("ProcessMessages(%s, %u bytes) : Exception '%s' caught, normally caused by a message being shorter than its stated length\n", strCommand.c_str(), ENVessageSize, e.what());
            }
        ...
        if (!fRet)
            printf("ProcessMessage(%s, %u bytes) FAILED\n", strCommand.c_str(), ENVessageSize);
    }
