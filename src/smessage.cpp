...
void ThreadSecureMsg(void* parg)
{
    ...
    RenameThread("Envycoin-smsg"); // Make this thread recognisable
...
void ThreadSecureMsgPow(void* parg)
{
    ...
    RenameThread("Envycoin-smsg-pow"); // Make this thread recognisable
    ...
    while (fSecMsgEnabled)
    {
        ...
        for (;;)
        {
            ...
            if (SecureMsgScaENVessage(pHeader, pPayload, psmsg->nPayload, true) != 0)
            {
...
bool SecureMsgReceiveData(CNode* pfrom, std::string strCommand, CDataStream& vRecv)
{
    ...
    */
    ...
    if (strCommand == "smsgWant")
    {
        ...
        vchBunch.resize(4+8); // ENVessages + bucketTime
    ...
    
    {
        ...
        if (nBuckets > 0) // no need to send keep alive pkts, coin messages already do that
        {
            ...
            for (it = smsgBuckets.begin(); it != smsgBuckets.end(); ++it)
            {
                ...
                uint32_t ENVessages = bkt.setTokens.size();
                
                if (bkt.timeChanged < pto->smsgData.lastMatched     // peer has this bucket
                    || ENVessages < 1)                               // this bucket is empty
                    continue; 
    ...
    for (fs::directory_iterator itd(pathSmsgDir) ; itd != itend ; ++itd)
    {
        ...
        
        {
            ...
            for (;;)
            {
                ...
                // -- don't report to gui, 
                int rv = SecureMsgScaENVessage(&smsg.hash[0], &vchData[0], smsg.nPayload, false);
...
int SecureMsgScaENVessage(unsigned char *pHeader, unsigned char *pPayload, uint32_t nPayload, bool reportToGui)
{
    /* 
    ...
    */
    ...
    if (fDebugSmsg)
        printf("SecureMsgScaENVessage()\n");
    
    if (pwalletMain->IsLocked())
    {
        if (fDebugSmsg)
            printf("ScaENVessage: Wallet is locked, storing message to scan later.\n");
    ...
    if (fOwENVessage)
    {
        ...
        try {
            ...
            printf("SecureMsgScaENVessage(): Could not resize vchData, %u, %s\n", SMSG_HDR_LEN + nPayload, e.what());
            return 1;
        };
...
int SecureMsgReceive(CNode* pfrom, std::vector<unsigned char>& vchData)
{
    ...
    for (uint32_t i = 0; i < nBunch; ++i)
    {
        ...
        if (SecureMsgScaENVessage(&vchData[n], &vchData[n + SMSG_HDR_LEN], psmsg->nPayload, true) != 0)
        {
            // message recipient is not this node (or failed)
        };
...
int SecureMsgEncrypt(SecureMessage& smsg, std::string& addressFrom, std::string& addressTo, std::string& message)
{
    ...
    */
    ...
    uint32_t leENVsgData;
    
    uint32_t leENVsg = message.size();
    if (leENVsg > 128)
    {
        ...
        int lenComp = LZ4_compress((char*)message.c_str(), (char*)&vchCompressed[0], leENVsg);
        if (lenComp < 1)
        {
    ...
    {
        try {
            vchPayload.resize(9 + leENVsgData);
        } catch (std::exception& e) {
            printf("vchPayload.resize %u threw: %s.\n", 9 + leENVsgData, e.what());
            return 8;
        };
        
        memcpy(&vchPayload[9], pMsgData, leENVsgData);
        
        vchPayload[0] = 250; // id as anonymous message
        // -- next 4 bytes are unused - there to ensure encrypted payload always > 8 bytes
        memcpy(&vchPayload[5], &leENVsg, 4); // length of uncompressed plain text
    } else
    {
        try {
            vchPayload.resize(SMSG_PL_HDR_LEN + leENVsgData);
        } catch (std::exception& e) {
            printf("vchPayload.resize %u threw: %s.\n", SMSG_PL_HDR_LEN + leENVsgData, e.what());
            return 8;
        };
        memcpy(&vchPayload[SMSG_PL_HDR_LEN], pMsgData, leENVsgData);
        ...
        memcpy(&vchPayload[1+20+65], &leENVsg, 4); // length of uncompressed plain text
    };
